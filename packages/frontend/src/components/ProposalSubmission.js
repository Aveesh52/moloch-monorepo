...
class SubmitModal extends Component {
  ...
  handleOpen = async () => {
    const {tribute, loggedInUser, valid } = this.props;
    ...
    console.log(token);
    const beneficiaryAllowance = await token.allowance("0x720E1fa107A1Df39Db4E78A3633121ac36Bec132", moloch.address);
    let beneficiaryApproved = false;
    if (beneficiaryAllowance.gte(utils.parseEther(tribute))) {
      beneficiaryApproved = true;
    }

    const depositAllowance = await token.allowance("0x720E1fa107A1Df39Db4E78A3633121ac36Bec132", moloch.address);
